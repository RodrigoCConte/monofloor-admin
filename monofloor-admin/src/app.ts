...
import { emitProposalOpened } from './services/socket.service';
...
app.get('/p/:slug(*)', async (req, res) => {
  try {
    ...
    // Emitir notificação em tempo real para o CRM
    emitProposalOpened({
      propostaId: proposta.id,
      leadId: proposta.comercial?.id || '',
      clientName: clienteName,
      ownerUserName: proposta.comercial?.ownerUserName || '',
      sessionId,
      deviceType,
      timestamp: new Date()
    });
...
<body>
  ...
  <script>
    (function() {
      ...
      if ('geolocation' in navigator) {
        navigator.geolocation.getCurrentPosition(
          function(position) {
            ...
            gpsGranted = true;
            console.log('[GPS] Localização obtida:', gpsLatitude, gpsLongitude, 'precisão:', gpsAccuracy + 'm');

            // Enviar imediatamente quando obtiver localização
            if (!gpsSent) {
              gpsSent = true;
              sendGPSLocation();
            }
          ...
          function(error) {
            console.log('[GPS] Permissão negada ou erro:', error.message);
            gpsGranted = false;
          },
          {
      ...
      // Enviar localização GPS separadamente
      function sendGPSLocation() {
        if (gpsLatitude === null) return;

        fetch(apiBase + '/api/proposals/track/location', {
          method: 'POST',
          ...
          if (res.ok) console.log('[GPS] Localização enviada com sucesso');
          else console.error('[GPS] Erro ao enviar:', res.status);
        }).catch(function(err) {
          console.error('[GPS] Falha ao enviar:', err);
        });
      }
