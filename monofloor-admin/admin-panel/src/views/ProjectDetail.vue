...
// Work hours options for dropdown (all 24 hours)
...
const addTeamMembers = async () => {
  ...
  try {
    // Add each selected applicator using their own role
    for (const userId of selectedApplicatorsToAdd.value) {
      // Find the applicator to get their role
      const applicator = availableApplicators.value.find((a: any) => a.id === userId);
      const role = applicator?.role || 'APLICADOR_I'; // Use applicator's role or default
...
const openPeopleModal = (task: any) => {
  ...
  // API returns { id, name, photoUrl, assignedAt } - so we use 'id' directly
