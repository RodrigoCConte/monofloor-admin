...
(async () => {
  ...
  await new Promise(r => setTimeout(r, 5000));
  await propostaPage.screenshot({ path: "/tmp/gerador-propostas.png" });
  console.log("7. Screenshot do gerador salvo");

  // Listar botões no gerador
  const buttons = await propostaPage.evaluate(() => {
    return Array.from(document.querySelectorAll("button")).map(b => b.textContent.trim());
  });
  console.log("8. Botões no gerador:", buttons);

  console.log("Mantendo aberto para você testar manualmente...");
