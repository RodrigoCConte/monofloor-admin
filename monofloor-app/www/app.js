...
function initSplashScreen() {
    ...
    // Ensure global nav is hidden on startup
    ...
    // Fallback: hide splash after 5 seconds even if video doesn't play
    setTimeout(() => {
        hideSplashScreen();
    }, 5000);

    // Also hide on video error
    splashVideo.addEventListener('error', () => {
        console.log('[Splash] Video error, hiding splash');
        hideSplashScreen();
    });
}
...
// =============================================
// KEYBOARD NAVIGATION (Enter para próximo campo)
// =============================================
document.addEventListener('DOMContentLoaded', () => {
    // Configurar navegação por Enter em todos os inputs
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Enter' && (e.target.tagName === 'INPUT' || e.target.tagName === 'SELECT')) {
            e.preventDefault();

            // Encontrar todos os campos focáveis na tela ativa
            const activeScreen = document.querySelector('.screen.active');
            if (!activeScreen) return;

            const focusableFields = activeScreen.querySelectorAll('input:not([type="hidden"]):not([disabled]), select:not([disabled]), textarea:not([disabled])');
            const currentIndex = Array.from(focusableFields).indexOf(e.target);

            if (currentIndex < focusableFields.length - 1) {
                // Próximo campo
                focusableFields[currentIndex + 1].focus();
            } else {
                // Último campo - fechar teclado
                e.target.blur();
            }
        }
    });
    ...
    contentAreas.forEach(content => {
        ...
        content.addEventListener('touchmove', (e) => {
            ...
            if (pullMoveY > 0 && content.scrollTop === 0) {
                ...
                const indicator = content.querySelector('.pull-refresh-indicator');
                if (indicator) {
                    indicator.style.transform = `translateY(${Math.min(pullMoveY / 2, 50)}px)`;
                    indicator.style.opacity = Math.min(pullMoveY / pullRefreshThreshold, 1);
                }
            }
        }, { passive: true });

        content.addEventListener('touchend', () => {
            if (isPulling && pullMoveY > pullRefreshThreshold) {
                // Executar refresh
                triggerPullRefresh();
            }
...
function triggerPullRefresh() {
    ...
    switch(screenId) {
        ...
        case 'screen-feed':
            ...
            break;
        case 'screen-hours':
            if (typeof loadHoursData === 'function') loadHoursData();
            break;
        case 'screen-project-detail':
            if (typeof loadProjectDetails === 'function' && selectedProject) {
                loadProjectDetails(selectedProject.id);
            }
            break;
        case 'screen-academia':
            if (typeof loadAcademiaContent === 'function') loadAcademiaContent();
            break;
        default:
            console.log('[PullRefresh] No refresh handler for screen:', screenId);
    }
}
...
// =============================================
// APP VISIBILITY (Background/Foreground detection)
// =============================================
let isAppInBackground = false;
let backgroundStartTime = null;

/**
 * Initialize app visibility detection for iOS background handling
 * When app goes to background, we pause GPS verification to avoid false "GPS off" detection
 */
function initAppVisibilityDetection() {
    // Set initial state based on current visibility
    isAppInBackground = document.hidden === true;
    console.log('[Visibility] Initial state: ' + (isAppInBackground ? 'BACKGROUND' : 'FOREGROUND'));

    // Standard Page Visibility API
    document.addEventListener('visibilitychange', () => {
        console.log('[Visibility] visibilitychange event - document.hidden:', document.hidden);
        if (document.hidden) {
    ...
    window.addEventListener('pagehide', () => {
        console.log('[Visibility] pagehide event');
        onAppWentToBackground();
    });

    window.addEventListener('pageshow', (event) => {
        console.log('[Visibility] pageshow event - persisted:', event.persisted);
        onAppCameToForeground();
    });

    // Capacitor App plugin events (if available) - THIS IS THE MAIN ONE FOR iOS
    if (window.Capacitor?.Plugins?.App) {
        console.log('[Visibility] Capacitor App plugin detected - adding appStateChange listener');
        window.Capacitor.Plugins.App.addListener('appStateChange', ({ isActive }) => {
            console.log('[Visibility] Capacitor appStateChange - isActive:', isActive);
            if (isActive) {
                onAppCameToForeground();
            } else {
                onAppWentToBackground();
        ...
        console.log('[Visibility] Capacitor App plugin NOT available');
    }

    console.log('[Visibility] App visibility detection initialized');
}

/**
 * Called when app goes to background
 */
function onAppWentToBackground() {
    if (isAppInBackground) return; // Already in background
    ...
    // Pause GPS off timer - don't count background time as "GPS off"
    if (gpsOffStartTime && activeCheckinId) {
        console.log('[Visibility] Pausing GPS off timer while in background');
        // Store the elapsed time before going to background
        window._gpsOffElapsedBeforeBackground = Date.now() - new Date(gpsOffStartTime).getTime();
    }
}
 ...
 */
function onAppCameToForeground() {
    if (!isAppInBackground) return; // Already in foreground

    const timeInBackground = backgroundStartTime ? Date.now() - backgroundStartTime : 0;
    isAppInBackground = false;
    backgroundStartTime = null;

    console.log(`[Visibility] App came to FOREGROUND (was in background for ${Math.round(timeInBackground/1000)}s)`);

    // If we had a GPS off timer, adjust it to not count background time
    if (gpsOffStartTime && window._gpsOffElapsedBeforeBackground !== undefined) {
        // Reset the gpsOffStartTime to now minus the elapsed time before background
        const adjustedStartTime = new Date(Date.now() - window._gpsOffElapsedBeforeBackground);
        gpsOffStartTime = adjustedStartTime.toISOString();
        console.log('[Visibility] Adjusted GPS off start time to exclude background time');
        delete window._gpsOffElapsedBeforeBackground;
    }

    // Force a fresh location check when coming back from background
    if (activeCheckinId && isCheckedIn) {
        console.log('[Visibility] Requesting fresh location after returning from background');
        forceLocationUpdate();
    }
}

/**
 * Force a fresh location update (used when returning from background)
 */
function forceLocationUpdate() {
    if (!navigator.geolocation) return;

    // Update the last position timestamp to prevent stale detection
    lastPositionTimestamp = Date.now();

    navigator.geolocation.getCurrentPosition(
        (position) => {
            console.log('[Visibility] Fresh location received:', position.coords.latitude, position.coords.longitude);
            lastPositionTimestamp = Date.now();
            onGeolocationSuccess(position);

            // If GPS was considered "off", mark it as back on
            if (gpsOffStartTime) {
                console.log('[Visibility] GPS is working - clearing GPS off state');
                logGPSOffEnd();
            }
        },
        (error) => {
            console.warn('[Visibility] Could not get fresh location:', error.message);
            // Don't immediately mark as GPS off - give it time
        },
        {
...
// Initialize Native Push Notifications (iOS/Android via Capacitor)
async function initNativePushNotifications() {
    console.log('[Push Native] Initializing native push notifications...');

    try {
        // Import PushNotifications from Capacitor
        const { PushNotifications } = window.Capacitor.Plugins;

        if (!PushNotifications) {
            console.error('[Push Native] PushNotifications plugin not available');
            return;
        }

        // Check current permission status
        let permStatus = await PushNotifications.checkPermissions();
        console.log('[Push Native] Current permission status:', permStatus.receive);

        if (permStatus.receive === 'prompt') {
            // Request permission
            permStatus = await PushNotifications.requestPermissions();
            console.log('[Push Native] Permission after request:', permStatus.receive);
        }

        if (permStatus.receive !== 'granted') {
            console.log('[Push Native] Permission not granted');
            return;
        }

        // Register for push notifications
        await PushNotifications.register();
        console.log('[Push Native] Registered for push notifications');

        // Listen for registration success
        await PushNotifications.addListener('registration', async (token) => {
            console.log('[Push Native] Device token received:', token.value);
            // Send token to server
            await sendNativeTokenToServer(token.value);
        });
        ...
        await PushNotifications.addListener('pushNotificationReceived', (notification) => {
            ...
            // Show in-app toast
            const title = notification.title || 'Monofloor';
        ...
        console.log('[Push Native] All listeners registered successfully');

    } catch (error) {
        console.error('[Push Native] Error initializing:', error);
    }
}

// Send native device token to server
async function sendNativeTokenToServer(deviceToken) {
    const token = getAuthToken();
    if (!token) {
        console.log('[Push Native] No auth token, cannot save device token');
        return;
    }
    ...
    try {
        ...
        if (response.ok) {
            console.log('[Push Native] Device token saved to server');
        } else {
            console.error('[Push Native] Failed to save device token:', await response.text());
        }
    } catch (error) {
        console.error('[Push Native] Error saving device token:', error);
    }
}

// Handle notification data (common handler)
function handleNotificationData(data) {
    if (data.type === 'CAMPAIGN' && data.campaignId) {
        // Clear campaign from seen cache and show it
        const seenCampaigns = JSON.parse(localStorage.getItem(CAMPAIGN_STORAGE_KEY) || '[]');
        const index = seenCampaigns.indexOf(data.campaignId);
        if (index > -1) {
            seenCampaigns.splice(index, 1);
            localStorage.setItem(CAMPAIGN_STORAGE_KEY, JSON.stringify(seenCampaigns));
        }
...
function handleNotificationTap(data) {
    ...
    if (data.type === 'CAMPAIGN' && data.campaignId) {
        // Navigate to campaigns and show specific campaign
        fetchCampaignsFromAPI().then(() => {
            const campaign = cachedCampaigns.find(c => c.id === data.campaignId);
            if (campaign) {
                showCampaign(campaign);
            }
        });
    ...
    } else if (data.type === 'REPORT_REMINDER') {
        // Navigate to project for report
        if (data.projectId) {
            navigateTo('screen-projects');
            setTimeout(() => {
                openProjectDetail(data.projectId);
...
// Register Service Worker and setup push notifications (Web Push fallback)
async function initPushNotifications() {
    // Check if running in native Capacitor environment
    if (isCapacitorNative()) {
        console.log('[Push] Running in Capacitor native, using native push');
        await initNativePushNotifications();
        return;
    }

    // Web Push (for browser)
    console.log('[Push] Running in browser, using Web Push');
...
function updateBottomNav(screenId) {
    ...
    const screensWithNav = [
        'screen-projects', 'screen-project-detail',
        'screen-feed', 'screen-new-post', 'screen-comments',
        'screen-hours', 'screen-learn', 'screen-profile',
        'screen-edit-profile', 'screen-badges', 'screen-report'
    ];
    ...
    if (globalNav) {
        ...
        // Update active state
        const globalItems = globalNav.querySelectorAll('.nav-item');
    ...
    // Legacy: update old bottom navs (if any are still visible)
    document.querySelectorAll('.screen .bottom-nav').forEach(nav => {
...
let geolocationState = 'loading'; // 'loading' | 'valid' | 'invalid' | 'error' | 'permission_denied' | 'unavailable' | 'no_project_coords'
let geolocationTimeout = null; // Timeout para detectar quando localização não está disponível
...
function getProjectCoordinates() {
    ...
    // Retornar null se projeto não tem coordenadas (não usar fallback)
    return null;
}
...
function updateLocationUI() {
    ...
    switch (geolocationState) {
        ...
        case 'unavailable':
            ...
            `;
            statusMain.textContent = 'Localização não compartilhada';
            statusMain.style.color = 'var(--accent-orange)';
            statusSub.textContent = 'Ative o GPS e permita o acesso à localização';
            if (checkinBtn) {
                checkinBtn.disabled = true;
                checkinBtn.classList.add('disabled');
            }
        ...
        case 'no_project_coords':
            ...
            `;
            statusMain.textContent = 'Projeto sem coordenadas';
            statusMain.style.color = 'var(--accent-orange)';
            statusSub.textContent = 'O projeto precisa ter endereço cadastrado';
            if (checkinBtn) {
                checkinBtn.disabled = true;
                checkinBtn.classList.add('disabled');
            }
...
function onGeolocationSuccess(position) {
    ...
    // DEBUG: Log para identificar bug de localização
    console.log('[DEBUG GPS] Minha localização:', currentUserLocation.lat, currentUserLocation.lon, 'precisão:', currentUserLocation.accuracy);

    const projectCoords = getProjectCoordinates();
    if (!projectCoords) {
        console.log('[DEBUG GPS] Projeto sem coordenadas!');
        geolocationState = 'no_project_coords';
        updateLocationUI();
        return;
    }

    console.log('[DEBUG GPS] Projeto coords:', projectCoords.lat, projectCoords.lon);
    ...
    console.log('[DEBUG GPS] Distância calculada:', currentDistanceToProject, 'm');
...
function startGeolocationWatch() {
    ...
    // Timeout para detectar quando localização não está sendo recebida
    if (geolocationTimeout) clearTimeout(geolocationTimeout);
    geolocationTimeout = setTimeout(() => {
        if (geolocationState === 'loading' && !currentUserLocation) {
            console.log('[Geolocation] Timeout - localização não recebida');
            geolocationState = 'unavailable';
            updateLocationUI();
        }
    }, 10000); // 10 segundos
    ...
    geolocationWatchId = navigator.geolocation.watchPosition(
        (position) => {
            // Limpar timeout pois recebemos localização
            if (geolocationTimeout) {
                clearTimeout(geolocationTimeout);
                geolocationTimeout = null;
            }
...
let cachedProjects = [];
// Cache de responsabilidade pelo relatório (carregado junto com projeto)
let isCurrentUserReportResponsible = false;
...
async function loadReportResponsibility(projectId) {
    ...
    try {
        ...
        if (data.success && data.data) {
            ...
            // Cache if current user is the report responsible (used during checkout)
            isCurrentUserReportResponsible = data.data.isCurrentUserResponsible || false;
            renderReportResponsibility(data.data);
            section.style.display = 'block';
        } else {
            // No responsibility assigned yet (before 6AM) - assume user IS responsible
            section.style.display = 'none';
            currentResponsibility = null;
            isCurrentUserReportResponsible = true; // Default to true if no assignment yet
        }
    } catch (error) {
        console.error('[Responsibility] Error loading:', error);
        section.style.display = 'none';
        isCurrentUserReportResponsible = true; // Default to true on error
    }
}
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (token) {
        ...
        // Initialize app visibility detection (background/foreground)
        initAppVisibilityDetection();
...
function renderTaskBlockSection(tasksData) {
    ...
    // Task block duration - use inputDays (configured duration) from the task block
    // Get the current task block (first group of pending tasks)
    const currentPhasePendingTasks = (tasksByPhase?.[currentPhase] || []).filter(t => t.status !== 'COMPLETED');

    // Find the first task block's duration and deadline (considering grouped tasks)
    let blockDurationDays = 0; // Sum of inputDays for the block
    let blockDaysRemaining = null; // For checking if overdue
    let blockTaskNames = [];
    if (currentPhasePendingTasks.length > 0) {
        // Get the first task and any tasks grouped with it
        const firstTask = currentPhasePendingTasks[0];
        blockTaskNames.push(firstTask.title);
        blockDurationDays = firstTask.inputDays || 1; // Use inputDays (configured duration)
        blockDaysRemaining = firstTask.daysRemaining;

        // Check if there are grouped tasks (groupWithNext)
        let i = 0;
        while (i < currentPhasePendingTasks.length - 1 && currentPhasePendingTasks[i].groupWithNext) {
            i++;
            const nextTask = currentPhasePendingTasks[i];
            blockTaskNames.push(nextTask.title);
            // Sum the inputDays for grouped tasks (they share the same duration)
            // Actually for grouped tasks, use the max inputDays since they're done together
            if (nextTask.inputDays && nextTask.inputDays > blockDurationDays) {
                blockDurationDays = nextTask.inputDays;
            }
            // Track the latest deadline for overdue check
            if (nextTask.daysRemaining !== null && (blockDaysRemaining === null || nextTask.daysRemaining > blockDaysRemaining)) {
                blockDaysRemaining = nextTask.daysRemaining;
            }
        }
    }

    const blockName = blockTaskNames.join(' + ');

    if (blockDurationDays > 0) {
        const days = blockDurationDays;
        const isPast = blockDaysRemaining !== null && blockDaysRemaining < 0;

        let deadlineMessage = '';
        let deadlineClass = '';

        if (isPast) {
            deadlineMessage = `Voce esta atrasado ${Math.abs(blockDaysRemaining)} dia${Math.abs(blockDaysRemaining) !== 1 ? 's' : ''} para concluir ${blockName}!`;
            deadlineClass = 'deadline-past';
        } else if (days === 1) {
            deadlineMessage = `Voce tem 1 dia para concluir ${blockName}`;
            deadlineClass = 'deadline-normal';
        } else {
            deadlineMessage = `Voce tem ${days} dias para concluir ${blockName}`;
            deadlineClass = 'deadline-normal';
        }

        html += `
            <div class="phase-deadline-countdown ${deadlineClass}">
                <span class="deadline-icon">${isPast ? '⚠️' : '📅'}</span>
    ...
    if (currentPhaseTasks.length === 0) {
        ...
        if (pendingTasks.length === 0) {
            ...
            // Group tasks that have groupWithNext=true
            const groupedTasks = [];
            let i = 0;
            while (i < pendingTasks.length) {
                const task = pendingTasks[i];
                const group = [task];
                // Keep adding tasks while current has groupWithNext
                while (task.groupWithNext && i + 1 < pendingTasks.length && pendingTasks[i].groupWithNext) {
                    i++;
                    group.push(pendingTasks[i]);
                }
            ...
            groupedTasks.forEach((taskGroup) => {
                // For grouped tasks, combine titles and use first task as primary
                const task = taskGroup[0];
                const combinedTitle = taskGroup.map(t => t.title).join(' + ');
                const groupTaskIds = taskGroup.map(t => t.id);
                ...
                // Task-specific duration (use inputDays - configured duration)
                let taskDeadlineHtml = '';
                const taskDuration = task.inputDays || 1;
                const isPast = task.daysRemaining !== null && task.daysRemaining < 0;
                if (isPast) {
                    taskDeadlineHtml = `<span class="task-deadline past">Atrasado ${Math.abs(task.daysRemaining)} dia${Math.abs(task.daysRemaining) !== 1 ? 's' : ''}</span>`;
                } else if (taskDuration === 1) {
                    taskDeadlineHtml = `<span class="task-deadline">1 dia</span>`;
                } else {
                    taskDeadlineHtml = `<span class="task-deadline">${taskDuration} dias</span>`;
                }

                // Store group task IDs as JSON for onclick
                const groupIdsJson = JSON.stringify(groupTaskIds).replace(/"/g, '&quot;');

                html += `
                    <div class="task-block-item-simple ${isPartial ? 'partial' : ''} ${isCura && isInProgress ? 'cura-active' : ''}"
                         data-task-id="${task.id}"
                         data-group-task-ids="${groupIdsJson}"
                         data-progress="${taskProgress}"
                         data-is-cura="${isCura}"
                         onclick="${isCura ? '' : `showTaskCompletionOptions('${task.id}', '${escapeHtml(combinedTitle).replace(/'/g, "\\'")}', ${taskProgress}, ${groupIdsJson.replace(/&quot;/g, "'")})`}">
                        <div class="task-color-bar" style="background-color: ${task.color || '#c9a962'}"></div>
                        <div class="task-info">
                            <div class="task-name">
                                ${isCura && isInProgress ? '<span class="cura-icon">⏳</span>' : ''}
                                ${escapeHtml(combinedTitle)}
                                ${taskDeadlineHtml}
                            </div>
 ...
 * @param {string} taskTitle - Display title (may be combined for grouped tasks)
 * @param {number} currentProgress - Current progress percentage
 * @param {string[]} groupTaskIds - Optional array of all task IDs in the group
 */
function showTaskCompletionOptions(taskId, taskTitle, currentProgress, groupTaskIds = null) {
    // Use groupTaskIds if provided, otherwise just the single taskId
    const taskIdsToComplete = groupTaskIds && groupTaskIds.length > 0 ? groupTaskIds : [taskId];
    const taskIdsJson = JSON.stringify(taskIdsToComplete);

    // Check if already at 100%
    if (currentProgress === 100) {
        // Toggle off - remove completion for all tasks in group
        taskIdsToComplete.forEach(id => completeTaskWithType(id, null, 0));
        return;
    }
    ...
    modal.innerHTML = `
        <div class="task-completion-options-modal">
            ...
            <div class="completion-options">
                <button class="completion-option integral" onclick="completeGroupedTasks(${taskIdsJson}, 'INTEGRAL', 100); closeTaskCompletionModal();">
                ...
                <button class="completion-option partial" onclick="showPartialCompletionSlider('${taskId}', ${taskIdsJson}); ">
                    <span class="option-icon">◐</span>
                    <span class="option-text">
            ...
            <div class="partial-slider-container" id="partialSliderContainer" style="display: none;">
                ...
                <button class="confirm-partial-btn" onclick="confirmPartialCompletionGrouped(${taskIdsJson});">
                    Confirmar Parcial
                </button>
            </div>

            ${currentProgress > 0 ? `
                <button class="reset-task-btn" onclick="completeGroupedTasks(${taskIdsJson}, null, 0); closeTaskCompletionModal();">
                    Resetar Tarefa
                </button>
            ` : ''}
...
/**
 * Complete multiple grouped tasks with the same completion type
 */
async function completeGroupedTasks(taskIds, completionType, progress) {
    for (const taskId of taskIds) {
        await completeTaskWithType(taskId, completionType, progress);
    }
}

/**
 * Confirm partial completion for grouped tasks
 */
function confirmPartialCompletionGrouped(taskIds) {
    const slider = document.getElementById('partialProgressSlider');
    const progress = slider ? parseInt(slider.value) : 50;
    completeGroupedTasks(taskIds, 'PARTIAL', progress);
    closeTaskCompletionModal();
}
 ...
 * Only the report responsible should see task completion and report options
 */
async function showTaskCompletionModal() {
    ...
    // Use cached responsibility status (loaded when project was opened)
    // If user is NOT the report responsible, do simple checkout
    if (!isCurrentUserReportResponsible) {
        console.log('[Checkout] User is not report responsible, doing simple checkout');
        const success = await doCheckoutWithReason(pendingCheckoutReason, true);
        if (success) {
            // Ensure we show a confirmation toast
            showToast('Check-out realizado com sucesso!');
        }
        return;
    }
    ...
    // Build the deadline message - use inputDays (configured duration) from the task block
    let deadlineHtml = '';
    if (currentProjectTasks?.tasksByPhase) {
        const currentPhase = currentProjectTasks.currentPhase || 'PREPARO';
        const currentPhasePendingTasks = (currentProjectTasks.tasksByPhase[currentPhase] || []).filter(t => t.status !== 'COMPLETED');

        // Get first task block duration and deadline (considering grouped tasks)
        let blockDurationDays = 0;
        let blockDaysRemaining = null;
        let blockTaskNames = [];
        if (currentPhasePendingTasks.length > 0) {
            const firstTask = currentPhasePendingTasks[0];
            blockTaskNames.push(firstTask.title);
            blockDurationDays = firstTask.inputDays || 1;
            blockDaysRemaining = firstTask.daysRemaining;

            // Include grouped tasks
            let i = 0;
            while (i < currentPhasePendingTasks.length - 1 && currentPhasePendingTasks[i].groupWithNext) {
                i++;
                const nextTask = currentPhasePendingTasks[i];
        ...
        if (blockDurationDays > 0) {
            ...
            if (isPast) {
                message = `Atrasado ${Math.abs(blockDaysRemaining)} dia${Math.abs(blockDaysRemaining) !== 1 ? 's' : ''} para concluir ${blockName}!`;
                cssClass += ' past';
            } else if (days === 1) {
                message = `Voce tem 1 dia para concluir ${blockName}`;
            } else {
                message = `Voce tem ${days} dias para concluir ${blockName}`;
            }
...
function loadMoreCheckoutTasks() {
    ...
    const listContainer = document.getElementById('todayTasksList');
    if (listContainer) {
        // Rebuild deadline HTML - use inputDays (configured duration) from the task block
        let deadlineHtml = '';
        if (currentProjectTasks?.tasksByPhase) {
            const currentPhase = currentProjectTasks.currentPhase || 'PREPARO';
            const currentPhasePendingTasks = (currentProjectTasks.tasksByPhase[currentPhase] || []).filter(t => t.status !== 'COMPLETED');
...
/**
 * Show all project tasks modal (includes tasks outside dependency sequence)
 */
function showAllProjectTasks() {
    ...
    // Use allUserTasks (all user's tasks without dependency filter) for "Other Tasks" modal
    // Falls back to allTasks for backward compatibility
    const allTasksSource = currentProjectTasks?.allUserTasks || currentProjectTasks?.allTasks;

    // DEBUG: Log what we have
    console.log('[DEBUG] showAllProjectTasks:');
    console.log('  allUserTasks count:', currentProjectTasks?.allUserTasks?.length);
    console.log('  allTasks count:', currentProjectTasks?.allTasks?.length);
    console.log('  currentTaskBlock count:', currentProjectTasks?.currentTaskBlock?.length);
    console.log('  allTasksSource count:', allTasksSource?.length);

    if (currentProjectTasks && allTasksSource) {
        const currentBlockIds = (currentProjectTasks.currentTaskBlock || []).map(t => t.id);
        console.log('  currentBlockIds:', currentBlockIds);

        // Show tasks that are not in current block AND not completed
        const otherTasks = allTasksSource.filter(t =>
            !currentBlockIds.includes(t.id) && t.status !== 'COMPLETED'
        );
        console.log('  otherTasks after filter:', otherTasks.length, otherTasks.map(t => t.title));

        allProjectTasksFromModal = otherTasks;

        if (otherTasks.length === 0) {
            // Show debug info to help troubleshoot
            const debugInfo = `allUserTasks: ${currentProjectTasks?.allUserTasks?.length || 0}, allTasks: ${currentProjectTasks?.allTasks?.length || 0}, currentBlock: ${currentBlockIds.length}`;
            listContainer.innerHTML = `
                <div class="empty-tasks-message">
                    <p>Nao ha outras tarefas disponiveis no projeto.</p>
                    <p style="font-size: 10px; color: #666; margin-top: 10px;">Debug: ${debugInfo}</p>
                </div>
            `;
        } else {
            let html = '';
...
function startGPSBackgroundVerification() {
    ...
    gpsVerificationInterval = setInterval(async () => {
        ...
        // IMPORTANT: Skip GPS verification when app is in background
        // iOS stops delivering location updates in background, which is normal behavior
        // We don't want to consider this as "GPS off"
        // Check both our flag AND document.hidden as fallback
        const actuallyInBackground = isAppInBackground || document.hidden === true;
        if (actuallyInBackground) {
            console.log('[GPS Check] Skipping - app is in background (flag:', isAppInBackground, ', document.hidden:', document.hidden, ')');
            return;
        }
        ...
        // BUT only if we're in foreground - background staleness is expected on iOS
...
async function openVideo(videoId) {
    ...
    if (youtubeId) {
        // Use YouTube embed iframe (more reliable in Capacitor/iOS)
        console.log('[Academy] Playing YouTube video:', youtubeId);
        videoPlayer.classList.add('hidden');

        const container = document.getElementById('youtubePlayerContainer');
        container.classList.remove('hidden');

        // Create YouTube embed iframe directly (works better in Capacitor)
        container.innerHTML = `
            <iframe
                id="youtubeIframe"
                src="https://www.youtube.com/embed/${youtubeId}?autoplay=1&rel=0&modestbranding=1&playsinline=1&enablejsapi=0"
                frameborder="0"
        ...
        // Hide loading and quality selector (not available with basic iframe)
        document.getElementById('videoLoadingOverlay').classList.add('hidden');
        document.getElementById('qualitySelector').style.display = 'none';

        // Start simple progress tracking (estimate based on video duration)
        if (currentVideo && currentVideo.durationSeconds) {
            let elapsed = 0;
            const trackingInterval = setInterval(() => {
                elapsed += 5;
                const percent = Math.min(100, Math.round((elapsed / currentVideo.durationSeconds) * 100));
                ...
                // Stop if screen changed
                if (!document.getElementById('screen-video-player').classList.contains('active')) {
                    clearInterval(trackingInterval);
                }
            }, 5000);
        }
